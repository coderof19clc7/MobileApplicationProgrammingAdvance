part of 'histories_cubit.dart';

@immutable
class HistoriesState extends WidgetState {
  final bool isLoadingMore;
  final int nextPage, total, currentTotal;
  final List<GroupedBookingInfo?> groupedHistoryBookingInfoList;

  @override
  List<Object?> get props => [
    ...super.props,
    isLoadingMore,
    nextPage,
    total,
    currentTotal,
    groupedHistoryBookingInfoList,
  ];

  @override
  WidgetState fromJson(json) {
    final mapJson = json as Map<String, dynamic>;
    return HistoriesState(
      isLoading: mapJson['isLoading'] as bool? ?? false,
      needNavigateToLogin: mapJson['needNavigateToLogin'] as bool? ?? false,
      basicStatusFToastState: mapJson['basicStatusFToastState'] != null
          ? BasicStatusFToastState.fromJson(mapJson['basicStatusFToastState'])
          : null,
      isLoadingMore: mapJson['isLoadingMore'] as bool? ?? false,
      nextPage: mapJson['nextPage'] as int? ?? 1,
      total: mapJson['total'] as int? ?? 0,
      currentTotal: mapJson['currentTotal'] as int? ?? 0,
      groupedHistoryBookingInfoList: mapJson['groupedHistoryBookingInfoList'] != null
          ? (mapJson['groupedHistoryBookingInfoList'] as List).map((e) {
            return e != null ? GroupedBookingInfo.fromJson(e) : null;
      }).toList()
          : [null, null, null],
      // groupedHistoryBookingInfoList: mapJson['groupedHistoryBookingInfoList'] != null
      //     ? (mapJson['groupedHistoryBookingInfoList'] as List<dynamic>)
      //         .map((e) => (e as List<dynamic>)
      //             .map((e) => e != null ? BookingInfo.fromJson(e) : null)
      //             .toList())
      //         .toList()
      //     : const <List<BookingInfo?>?>[null, null, null],
    );
  }

  @override
  Map<String, dynamic> toJson() {
    final map = <String, dynamic>{};
    map['isLoading'] = isLoading;
    map['needNavigateToLogin'] = needNavigateToLogin;
    if (basicStatusFToastState != null) {
      map['basicStatusFToastState'] = basicStatusFToastState?.toJson();
    }
    map['isLoadingMore'] = isLoadingMore;
    map['nextPage'] = nextPage;
    map['total'] = total;
    map['currentTotal'] = currentTotal;
    map['groupedHistoryBookingInfoList'] = groupedHistoryBookingInfoList
        .map((e) => e?.bookingInfoList?.map((e) => e?.toJson()).toList())
        .toList();
    return map;
  }

//<editor-fold desc="Data Methods">
  const HistoriesState({
    super.isLoading,
    super.needNavigateToLogin,
    super.basicStatusFToastState,
    this.isLoadingMore = false,
    this.nextPage = 1,
    this.total = 0,
    this.currentTotal = 0,
    this.groupedHistoryBookingInfoList = const <GroupedBookingInfo?>[null, null, null],
  });

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
          (other is HistoriesState &&
              runtimeType == other.runtimeType &&
              isLoading == other.isLoading &&
              needNavigateToLogin == other.needNavigateToLogin &&
              basicStatusFToastState == other.basicStatusFToastState &&
              isLoadingMore == other.isLoadingMore &&
              nextPage == other.nextPage &&
              total == other.total &&
              currentTotal == other.currentTotal &&
              listEquals(groupedHistoryBookingInfoList, other.groupedHistoryBookingInfoList));

  @override
  int get hashCode =>
      isLoading.hashCode ^
      needNavigateToLogin.hashCode ^
      basicStatusFToastState.hashCode ^
      isLoadingMore.hashCode ^
      nextPage.hashCode ^
      total.hashCode ^
      currentTotal.hashCode ^
      groupedHistoryBookingInfoList.hashCode;

  @override
  String toString() {
    return 'HistoriesState{'
        ' isLoading: $isLoading,'
        ' needNavigateToLogin: $needNavigateToLogin,'
        ' basicStatusFToastState: $basicStatusFToastState,'
        ' isLoadingMore: $isLoadingMore,'
        ' nextPage: $nextPage,'
        ' total: $total,'
        ' currentTotal: $currentTotal,'
        ' groupedHistoryBookingInfoList: $groupedHistoryBookingInfoList,'
        ' }';
  }

  HistoriesState copyWith({
    bool? isLoading,
    bool? needNavigateToLogin,
    BasicStatusFToastState? basicStatusFToastState,
    bool? isLoadingMore,
    bool? isLoadingTotalCall,
    int? nextPage,
    int? totalCall,
    int? total,
    int? currentTotal,
    List<GroupedBookingInfo?>? groupedHistoryBookingInfoList,
  }) {
    return HistoriesState(
      isLoading: isLoading ?? this.isLoading,
      needNavigateToLogin: needNavigateToLogin ?? this.needNavigateToLogin,
      basicStatusFToastState: basicStatusFToastState ?? this.basicStatusFToastState,
      isLoadingMore: isLoadingMore ?? this.isLoadingMore,
      nextPage: nextPage ?? this.nextPage,
      total: total ?? this.total,
      currentTotal: currentTotal ?? this.currentTotal,
      groupedHistoryBookingInfoList: groupedHistoryBookingInfoList ?? this.groupedHistoryBookingInfoList,
    );
  }

  Map<String, dynamic> toMap() {
    return {
      'isLoading': isLoading,
      'needNavigateToLogin': needNavigateToLogin,
      'basicStatusFToastState': basicStatusFToastState?.toMap(),
      'isLoadingMore': isLoadingMore,
      'nextPage': nextPage,
      'total': total,
      'currentTotal': currentTotal,
      'groupedHistoryBookingInfoList': groupedHistoryBookingInfoList.map(
            (e) => e?.bookingInfoList?.map((e) => e?.toMap()).toList(),
      ).toList(),
    };
  }

  factory HistoriesState.fromMap(Map<String, dynamic> map) {
    return HistoriesState(
      isLoading: map['isLoading'] as bool? ?? false,
      needNavigateToLogin: map['needNavigateToLogin'] as bool? ?? false,
      basicStatusFToastState: map['basicStatusFToastState'] != null
          ? BasicStatusFToastState.fromMap(map['basicStatusFToastState'] as Map<String, dynamic>)
          : null,
      isLoadingMore: map['isLoadingMore'] as bool? ?? false,
      nextPage: map['nextPage'] as int? ?? 1,
      total: map['total'] as int? ?? 0,
      currentTotal: map['currentTotal'] as int? ?? 0,
      groupedHistoryBookingInfoList: map['groupedHistoryBookingInfoList'] != null
          ? (map['groupedHistoryBookingInfoList'] as List).map((e) {
        return e != null ? GroupedBookingInfo.fromMap(e as Map<String, dynamic>) : null;
      }).toList()
          : [null, null, null],
    );
  }

//</editor-fold>
}
